...
<script>
  // 100dvh fix for iOS Safari: update --vh on resize/orientation
  (function(){
    function setVH(){
...
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<title>Трекер привычек — тёмная тема</title>
<style>
    :root{
      --bg:#0b0c0f;
      --panel:#0f1114;
      --muted:#9aa0a6;
      --accent:#ff6b3d; /* оранжево-красный */
      --accent-2:#ff3b30;
...
/* Adjust layout on keyboard open */
body.kb-open nav.bottom{ position: static !important; } /* avoid fixed jank under keyboard */
body.kb-open .app{ height: auto !important; min-height: calc(var(--vh, 1vh) * 100); }
body.kb-open .chat{ height: auto !important; }
...
body.kb-open nav.bottom{
  position: fixed !important;
  left: 0; right: 0;
  /* push nav below the visible area by keyboard height (when available) */
  ...
  <main>
    ...
    <section class="view" id="view-habit">
      ...
      <div class="habit-card" id="trash-panel" style="display:none;">
        <div style="font-weight:700; margin-bottom:8px">Корзина</div>
        <div class="sub" id="trash-list">Пусто</div>
      </div>
    </section>
    ...
    <section class="view hidden" id="view-chat">
      <div class="chat" style="height:100%">
        ...
        <div class="chat-input">
          ...
          <button class="icon-btn" id="send-btn">Отправить</button>
        </div>
      </div>
      <div class="habit-card" id="trash-panel-chat" style="margin-top:12px; display:none;">
        <div style="font-weight:700; margin-bottom:8px">Корзина</div>
        <div class="sub" id="trash-list-chat">Пусто</div>
      </div>
    </section>
    ...
    <section class="view hidden" id="view-profile">
      <div class="profile">
        ...
        <h3 style="margin-top:16px">Награды</h3>
...
// Жёсткий системный промпт (правила из ТЗ) + строгий JSON-формат ответа
const SYSTEM_PROMPT = `
Ты — помощник в приложении для создания привычек.
Строго следуй правилам:
1) Сначала уточняется конкретная привычка с измеримой целью.
2) Уточняется срок внедрения (≥1 месяц; для отказа от вредной — ≥2 месяца).
3) Уточняется текущий уровень пользователя.
4) Составь помодневный план на весь срок, деля на 4 четверти:
   Q1: 15–20% цели; Q2: 40–45%; Q3: 70–75%; Q4: 75–100% (последний день = 100%).
   • 1-я неделя «смешно лёгкая», первые 2–3 дня — подготовка.
   • Каждый день уникален.
   • Рост плавный: сначала маленькие прибавки, в конце больше.
   • Только конкретные цифры.
5) Пользователь выполняет под свою ответственность.
6) В ЭТОМ ВЫВОДЕ советов не давать.

Выводи ТОЛЬКО JSON строго этого вида (без текста/markdown):
{
  "habit": "строка",
  "duration_days": <число>,
  "start_date": "YYYY-MM-DD",
  "plan": [
    { "day": 1, "task": "строковое задание дня 1" },
    ...
...
async function generatePlanForHabit(h){
  ...
  // Если Mistral прислал валидный план – кладём его в привычку
  if(data && Array.isArray(data.plan)){
    ...
    // Фолбэк: простая «лестница» если сеть/модель не ответила валидно
    h.plan = {
      start: h.start,
      items: Array.from({length: h.targetDays}, (_,i)=>({
        day: i+1,
        text: `Мини-шаг №${i+1} для «${h.name}»`
      }))
    };
    saveState(state); renderAll();
  }
...
// Утилита: получить текст задания на сегодня для привычки
function getTodayTaskText(h){
  ...
  function createHabit(name){
    ...
    const n = (name||'Новая привычка').trim();
    const id = uid();
const h = {
  ...
  function renderCalendar(h, wrapper, ym){
    ...
    function draw(y, m){
      ...
      for(let d=1; d<=last.getDate(); d++){
        ...
        if(dateStr === todayKey){
          cell.style.cursor='pointer'; cell.title='Отметить выполнение за сегодня';
          ...
          cell.style.pointerEvents='none'; cell.title='Можно отмечать только текущий день'; cell.style.opacity = 0.85;
        }
  ...
  function renderProfile(){
    try{
      // Name
      profileNameEl.textContent = state?.profile?.name || 'Гость';
      // Avatar
  ...
  function renderTrash(){
    ...
    state.trash.forEach(h=>{
      ...
      const btn = document.createElement('button'); btn.className='icon-btn'; btn.textContent='Восстановить';
  ...
  function renderAll(){
    ...
    if(!state.habits.length){
      ...
      empty.className='sub';
      empty.textContent='Нет привычек. Добавьте через чат ниже: напишите название, затем укажите срок (в днях).';
...
[...incomplete, ...complete].forEach(h=>{
      ...
      const durationEl = document.createElement('div'); durationEl.className='sub'; durationEl.textContent = `Срок: ${total} дн.`;
      titleWrap.appendChild(nameEl); titleWrap.appendChild(durationEl);

      const right = document.createElement('div'); right.style.textAlign='right';
      const subProg = document.createElement('div'); subProg.className='sub'; subProg.textContent='Прогресс';
      ...
      const delBtn = document.createElement('button'); delBtn.className='icon-btn'; delBtn.title='Удалить привычку'; delBtn.textContent='🗑️';
      delBtn.style.marginLeft='8px';
      delBtn.addEventListener('click', ()=>{ if(confirm('Удалить привычку в корзину?')) deleteHabit(h.id); });
      ...
      const sub = document.createElement('div'); sub.className='sub'; sub.textContent = `${done} / ${total} выполнено`;
...
tt.textContent = todayText || 'Сегодня';

const ts = document.createElement('div');
ts.className='sub';
ts.textContent = `День ${todayIdx} из ${total}`;
      const ts = document.createElement('div'); ts.className='sub'; ts.textContent = doneToday ? 'Сегодня — ✔' : 'Сегодня — ✖';
      ti.appendChild(tt); ti.appendChild(ts);
      const muted = document.createElement('div'); muted.className='muted'; muted.textContent = doneToday ? 'Выполнено' : 'Не выполнено';
      ...
      const toggleBtn = document.createElement('button'); toggleBtn.className='icon-btn'; toggleBtn.textContent='Календарь';
...
listRoot.appendChild(card);
  ...
  function parseHabitIntent(text){
    ...
    let name = clean
      .replace(/^(добав(ь|и)|добавить|создай|создать|прибавь|сделай)\s*/i,'')
      .replace(/\bна\s+\d{1,3}\s*(дн(я|ей)?|дня|дней)?\b/gi,'')
      .replace(/\b\d{1,3}\s*(дн(я|ей)?|дня|дней)?\b/gi,'')
      .replace(/\bдн(я|ей)?\b/gi,'')
      .replace(/\bдень\b/gi,'')
      .replace(/\s+/g,' ').trim();

    // If message looks like a habit name (2..40 chars) and not only filler words
    const looksLikeName = /^[\wА-Яа-яЁё\s.,!()\-\u2013\u2014]{2,40}$/.test(name || '') && !/дн|дней|day|days/i.test(name || '');

    if(looksLikeName){
      return { name, days };
    }
    // If the whole text looks like a name (without cleaning)
    const asName = /^[\wА-Яа-яЁё\s.,!()\-\u2013\u2014]{2,40}$/.test(clean) && !/дн|дней|day|days/i.test(clean);
    if(asName){
...
if(state.pending && state.pending.type==='duration' && state.pending.habitId){
  ...
  if(num){
    ...
    pushMsg(`Ок, срок: ${n} дней. Скажи текущий уровень: сколько ты делаешь сейчас (число + единица)?`, 'bot');
    state.pending = { type:'baseline', habitId: state.pending.habitId }; saveState(state);
    return;
  } else {
    pushMsg('Напиши число дней, например: 30', 'bot');
    return;
  }
}
if(state.pending && state.pending.type==='baseline' && state.pending.habitId){
  const h = state.habits.find(x=>x.id===state.pending.habitId);
  if(h){
    h.baseline = text.trim();  // храним как строку («200 мл», «5 страниц» и т.п.)
    saveState(state);
    pushMsg('Супер! План сформирую и добавлю в календарь.', 'bot');
    // генерим помодневный план молча и кидаем в календарь:
    generatePlanForHabit(h);
  }
  ...
  return;
    ...
    if(parsed && parsed.name){
      ...
      if(parsed.days){
        ...
        pushMsg('Какой у тебя сейчас уровень по этой привычке (число + единица)?', 'bot');
state.pending = { type:'baseline', habitId: h.id }; saveState(state);
return;

        saveState(state); renderAll();
        pushMsg(`Добавил: «${h.name}» на ${parsed.days} дней. Удачи!`, 'bot');
        showView('view-habit');
        return;
      } else {
        pushMsg(`Добавил: «${h.name}». На сколько дней планируешь? Напиши число, например: 30`, 'bot');
    ...
    if(state.pending && state.pending.type==='name'){
      const name = text.replace(/^привыч(ка|ку)?\s*/i, '').trim() || 'Новая привычка';
    ...
    if(numInline){
      ...
      if(h){
        ...
        pushMsg(`Ок, срок для «${h.name}»: ${n} дней.`, 'bot');
        return;
      }
    }

    const looksLikeName = /^[\wА-Яа-яЁё\s.,!()\-\u2013\u2014]{2,40}$/.test(text) && !/дн|дней|day|days/i.test(text);
    if(looksLikeName){
    ...
    if(!state.habits || state.habits.length===0){
      pushMsg('Что добавить в привычки? Просто напиши короткое название, например: «Пить воду».', 'bot');
      state.pending = { type:'name' }; saveState(state);
    } else {
      pushMsg('Напиши название привычки, которую добавить. Можно сразу указать срок: «Пить воду 30».', 'bot');
      state.pending = { type:'name' }; saveState(state);
    }
  }
  ...
  try{
    ...
    if(!askedBefore){
      const nm = prompt('Как тебя зовут?');
      if(nm && nm.trim()){
  ...
  try{
    ...
    if(!was){
      pushMsg('Привет! Напиши короткое название привычки — я добавлю её и спрошу срок (в днях).', 'bot');
...
(function(){
  ...
  function ensureBadgeState(){
    try{
      ...
      if(!raw) return null;
      const s = JSON.parse(raw);
      if(!s.profile) s.profile = {name:"Гость", avatar:"", streak:0};
  ...
  function renderBadges(state){
    ...
    function createBadge(type, n, unlocked){
      ...
      d.innerHTML = `<div class="aura"></div>${badgeSVG(type, n)}<div class="label">${type==='streak'?'серия':'привычки'}</div><div class="value">${type==='streak'? n : n}</div>`;
      return d;
    }
    ...
    if(newEarned.length){
      ...
      showToast(type==='streak' ? `Новый серия: ${n} дней!` : `Новая награда: ${n} привыч${n===1?'ка':'ки'}`);
...
<div class="achv-backdrop" id="achv-backdrop" aria-hidden="true" style="display:none">
  <div class="achv-modal" role="dialog" aria-modal="true" aria-labelledby="achv-title">
    <div class="achv-header">
      ...
      <div class="achv-title" id="achv-title">Поздравляем! Новое достижение</div>
    </div>
    <div class="achv-body" id="achv-text">Вы разблокировали награду.</div>
    <div class="achv-actions">
      <button class="btn" id="achv-close">Закрыть</button>
      <button class="btn primary" id="achv-ok">Класс!</button>
    </div>
  </div>
</div>
...
(function(){
  ...
  function openModal(m){const b=document.getElementById('achv-backdrop');const t=document.getElementById('achv-text');if(!b||!t)return;t.textContent=m||'Вы разблокировали награду.';b.style.display='flex';b.classList.add('show');document.body.classList.add('modal-open');}
  ...
  function ensureChip(){
    const p=document.getElementById('view-profile'); if(!p) return;
    const name=p.querySelector('.name');
    let h=document.getElementById('series-chip-holder');
    if(!h){ h=document.createElement('div'); h.id='series-chip-holder'; h.className='series-chip-place'; if(name&&name.parentElement) name.parentElement.after(h); else p.prepend(h); }
    let c=document.getElementById('series-chip-gold');
    if(!c){ c=document.createElement('span'); c.id='series-chip-gold'; c.className='series-chip-gold'; c.innerHTML='<span class="flame">🔥</span><span class="series-text">Серия 0 дней</span>'; h.innerHTML=''; h.appendChild(c); }
    // cleanup duplicates
    p.querySelectorAll('#streak-chip-el').forEach(n=>n.remove());
    h.querySelectorAll('span').forEach(n=>{ if(n.id!=='series-chip-gold') n.remove(); });
  }
  function updateChip(){
    const s=getState()||{}; const c=document.getElementById('series-chip-gold'); if(!c) return;
    const t=c.querySelector('.series-text') || querySelector('.txt'); const v=(s.profile?.streak)||0; if(t) t.textContent='Серия '+v+' '+decl(v,['день','дня','дней']);
  }

  document.addEventListener('click',function(e){
    const today=!!e.target.closest('.today-check,.check');
    const nav=e.target.closest('.nav-btn');
    if(!today && !(nav&&nav.dataset&&nav.dataset.view==='view-profile'))return;

    const date=isoDate(new Date());
    let prevSnap=null, prevMaster=null, prevStreak=null;
    if(today){
      const sp=getState()||{}; prevSnap=snapshotHabits(sp); prevMaster=new Set((sp.profile?.badges?.mastered)||[]); prevStreak=new Set((sp.profile?.badges?.streak)||[]);
    }

    setTimeout(function(){
      ensureChip(); updateChip();
      if(today){
        const s=getState()||{}; const next=snapshotHabits(s);
        const log=getLog(); log.mastered[date]=log.mastered[date]||{}; log.streak[date]=log.streak[date]||{};

        // mastered: last day only, once per day per habit
        let doneId=null;
        next.forEach((n,id)=>{
          const p=prevSnap.get(id); if(!p) return;
          const d=n.count-p.count;
          const tw=p.set.has(date), tn=n.set.has(date);
          const fin=!!n.target && n.count===n.target;
          if(d===1 && !tw && tn && fin && !log.mastered[date][id]) doneId=id;
        });
        if(doneId){
          log.mastered[date][doneId]=true; setLog(log);
          const pc=[...prevSnap.values()].filter(v=>v.target&&v.count>=v.target).length;
          const nc=[...next.values()].filter(v=>v.target&&v.count>=v.target).length;
          if(nc>pc){
            const s2=getState()||{}; const setM=new Set((s2.profile?.badges?.mastered)||[]);
            let add=0; for(const st of MASTERED_STEPS){ if(nc>=st && !prevMaster.has(st)) add=st; }
            if(add){ setM.add(add); s2.profile=s2.profile||{}; s2.profile.badges=s2.profile.badges||{}; s2.profile.badges.mastered=[...setM].sort((a,b)=>a-b); setState(s2); if(typeof window.renderBadges==='function') window.renderBadges(s2); }
          }
          openModal('Поздравляем! Привычка завершена 🎉');
        }

        // streak: trigger once per day per threshold
        const sNow=getState()||{}; const cur=(sNow.profile?.streak)||0;
        let addS=0; for(const n of STREAK_STEPS){ if(cur>=n && !prevStreak.has(n)) addS=n; }
        if(addS && !log.streak[date][addS]){
          log.streak[date][addS]=true; setLog(log);
          const setS=new Set((sNow.profile?.badges?.streak)||[]); setS.add(addS);
          sNow.profile=sNow.profile||{}; sNow.profile.badges=sNow.profile.badges||{}; sNow.profile.badges.streak=[...setS].sort((a,b)=>a-b); setState(sNow);
          if(typeof window.renderBadges==='function') window.renderBadges(sNow);
          openModal('Поздравляем! Новая серия: '+addS+' '+decl(addS,['день','дня','дней']));
        }
      }
  ...
  document.addEventListener('DOMContentLoaded', function(){ ensureChip(); updateChip(); });
})();
</script>
<!-- === /PATCH === -->
...
(function(){
  ...
  function updateGoldChipText(){
    const chip = document.getElementById('series-chip-gold');
    const s = getState() || {};
    const v = (s.profile && typeof s.profile.streak==='number') ? s.profile.streak : 0;
    if(chip){
      let txt = chip.querySelector('.series-text') || chip.querySelector('.series-text') || querySelector('.txt');
      if(txt){ txt.textContent = 'Серия ' + v + ' ' + decl(v, ['день','дня','дней']); }
    }
  }
  ...
  function ensureGoldChip(){
    ...
    if(!chip){
      ...
      chip.innerHTML = '<span class="flame">🔥</span><span class="series-text">Серия 0 дней</span>';
      holder.innerHTML = '';
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    // Recompute after toggling "Сегодня"
    document.addEventListener('click', function(e){
...
(function(){
  ...
  function isoDate(d){const t=new Date(d.getFullYear(),d.getMonth(),d.getDate());return t.toISOString().slice(0,10);}
  function decl(n,f){n=Math.abs(n)%100;const n1=n%10;if(n>10&&n<20)return f[2];if(n1>1&&n1<5)return f[1];if(n1==1)return f[0];return f[2];}
  function ensureFields(s){s.profile=s.profile||{name:'Гость',avatar:'',streak:0};s.profile.badges=s.profile.badges||{streak:[],mastered:[]};}
  function snapshotHabits(s){const m=new Map();(s?.habits||[]).forEach(h=>{const t=h?.targetDays?Math.max(1,h.targetDays):null;const set=new Set(Array.isArray(h?.days)?h.days.slice():[]);m.set(h.id,{target:t,set,count:set.size});});return m;}
  function masteredCount(snap){let c=0;snap.forEach(v=>{if(v.target&&v.count>=v.target)c++;});return c;}
  ...
  /* ==== modal ==== */
  function openModal(m){const b=document.getElementById('achv-backdrop'),t=document.getElementById('achv-text');if(!b||!t)return;t.textContent=m||'Вы разблокировали награду.';b.style.display='flex';b.classList.add('show');document.body.classList.add('modal-open');}
  function closeModal(){const b=document.getElementById('achv-backdrop');if(!b)return;b.classList.remove('show');b.style.display='none';document.body.classList.remove('modal-open');}
  document.addEventListener('keydown',e=>{if(e.key==='Escape')closeModal();});
  document.getElementById('achv-ok')?.addEventListener('click',closeModal,{passive:true});
  document.getElementById('achv-close')?.addEventListener('click',closeModal,{passive:true});
...
<!-- === PATCH: Yandex fix — remove streak before awards + inline editable name (no observers) === -->
<style>
  ...
  #view-profile .name, #profile-name { cursor: text; border-bottom: 1px dashed rgba(255,255,255,.25); }
</style>
<script>
(function(){
  try{
    ...
    function wireEditableName(){
      ...
      nameEl.title = 'Нажмите, чтобы изменить имя';
...
<script>
/*
Mistral AI integration for your habit tracker (no external calendar services).
Drop this <script> AFTER your existing big <script> block, right before </body>.
It adds:
- callMistral(): fetches a plan from Mistral (or your proxy)
- addPlanToCalendar(): writes generated tasks into the built‑in calendar
- Minimal UI tweaks (planned vs done markers)
- A new command in chat: any user message will be sent to Mistral using your prompts
  and the bot will silently add tasks to the calendar (no plan text is sent back)

Assumptions about Mistral response shape (adjust in parseMistralPlan):
{
  "habits": [
    { "name": "Пить воду", "dates": ["2025-10-06","2025-10-07", ...] },
    { "name": "Зарядка",  "dates": ["2025-10-06","2025-10-08"] }
  ]
}
*/
(function(){
  // 1) === State extensions ===
  // Ensure each habit has a `plan` array of ISO dates separate from `days` (completed)
  function ensurePlanArrays(){
    try{
      const raw = localStorage.getItem('habit_traker_demo_v3');
      if(!raw) return;
      const s = JSON.parse(raw);
      (s.habits||[]).forEach(h=>{ if(!Array.isArray(h.plan)) h.plan = []; });
      localStorage.setItem('habit_traker_demo_v3', JSON.stringify(s));
  ...
  // 2) === Calendar visuals: planned vs done ===
  // Patch existing renderCalendar to show planned days differently (outline) and done days (filled)
  // This wrapper expects your original renderCalendar to be defined.
  const _renderCalendar = window.renderCalendar;
  if(_renderCalendar){
    window.renderCalendar = function(h, wrapper, ym){
      // delegate to original to build structure
      _renderCalendar(h, wrapper, ym);
      // Then decorate the current month grid we just drew
      // We target little 8px dot inside each cell; if day is planned but not done -> outline
      try{
        const dots = wrapper.querySelectorAll('.cal-grid .cal-cell');
        const todayISO = new Date(); todayISO.setHours(0,0,0,0);
        const base = new Date(todayISO.getFullYear(), todayISO.getMonth(), 1);
        const monthName = wrapper.querySelector('.cal-head .muted')?.textContent||'';
        // Nothing else to compute here; we simply read the number printed in the cell
        (dots||[]).forEach(cell=>{
          const day = cell.querySelector('.day');
          if(!day) return; // header row
          const m = new Date(wrapper.querySelector('.cal-head .muted').textContent);
          // Parse month label via locale is unreliable; rebuild ISO by walking child index
          // Simpler: infer Y/M from the previous sibling that holds month/year via dataset (we add below)
        });
      }catch(e){}
    };
  }

  // Alternative: lightly hook cell creation by observing DOM mutations once per draw
  // and decorating planned cells. This works without replacing your existing code.
  const iso = d=>{ const t=new Date(d.getFullYear(), d.getMonth(), d.getDate()); return t.toISOString().slice(0,10); };
  const observeCalendar = new MutationObserver(()=>{
    try{
      const raw = localStorage.getItem('habit_traker_demo_v3'); if(!raw) return;
      const s = JSON.parse(raw);
      // For every card/calendar pair, compute its current shown month from the label
      document.querySelectorAll('.habit-card').forEach(card=>{
        const name = card.querySelector('.habit-title div:first-child div:first-child')?.textContent?.trim();
        if(!name) return;
        const cal = card.querySelector('.calendar:not(.hidden)');
        if(!cal) return;
        const label = cal.querySelector('.cal-head .muted');
        const grid  = cal.querySelector('.cal-grid');
        if(!label || !grid) return;
        const [monthName, yearStr] = (label.textContent||'').split(/\s+/);
        const ruMonths = ['января','февраля','марта','апреля','мая','июня','июля','августа','сентября','октября','ноября','декабря'];
        const ruMonths2 = ['январь','февраль','март','апрель','май','июнь','июль','август','сентябрь','октябрь','ноябрь','декабрь'];
        let mIndex = ruMonths.indexOf((monthName||'').toLowerCase());
        if(mIndex<0) mIndex = ruMonths2.indexOf((monthName||'').toLowerCase());
        const Y = parseInt(yearStr,10); if(isNaN(Y) || mIndex<0) return;
        const habit = (s.habits||[]).find(h=>h.name===name);
        if(!habit) return;
        const planSet = new Set(habit.plan||[]);
        const doneSet = new Set(habit.days||[]);
        // Walk cells for days 1..31 and mark states
        const cells = Array.from(grid.children).filter(el=>el.classList.contains('cal-cell'));
        cells.forEach(cell=>{
          const dEl = cell.querySelector('.day');
          if(!dEl) return; // skip header row fillers
          const d = parseInt(dEl.textContent,10); if(isNaN(d)) return;
          const key = iso(new Date(Y, mIndex, d));
          let dot = cell.querySelector('div:nth-child(2)'); // the 8px dot
          if(!dot){ dot = document.createElement('div'); dot.style.height='8px'; dot.style.width='8px'; dot.style.borderRadius='6px'; dot.style.marginTop='6px'; cell.appendChild(dot); }
          if(doneSet.has(key)){
            cell.classList.add('done');
            dot.style.background='var(--success)';
            dot.style.outline='none';
          } else if(planSet.has(key)){
            // planned but not done -> outlined dot
  ...
  observeCalendar.observe(document.body, {childList:true, subtree:true});

  // 3) === Mistral client ===
  // Choose ONE mode below: direct or proxied. Proxied is safer (no key in frontend).
  const MISTRAL_MODE = 'proxy'; // 'direct' | 'proxy'

  async function callMistral(userText){
    // IMPORTANT: you said prompts already exist. Inject them here.
    const systemPrompt = "Ты помощник по привычкам. Получи сообщение пользователя и сгенерируй план из дат на ближайшие 4 недели. Верни JSON только в этом формате: {\"habits\":[{\"name\":\"...\",\"dates\":[\"YYYY-MM-DD\", ...]}]} без лишнего текста.";

    if(MISTRAL_MODE==='direct'){
      // Direct call to Mistral API (requires your API key; not recommended in frontend)
      const apiKey = "PUT_YOUR_KEY_HERE"; // ⚠️ replace or switch to proxy mode
      ...
      // Proxy mode: implement POST /api/mistral on your backend that forwards to Mistral
      ...
      const data = await res.json();
      // Expect { content: "{\"habits\":[...] }" }
      return data.content || JSON.stringify(data);
    }
  }

  function parseMistralPlan(text){
    try{
      const obj = JSON.parse(text);
      if(obj && Array.isArray(obj.habits)) return obj.habits.map(h=>({ name: String(h.name||'').trim(), dates: Array.isArray(h.dates)? h.dates.filter(Boolean):[] }));
    }catch(e){}
    return [];
  }
  ...
  function addPlanToCalendar(plan){
    ...
    function findByName(name){ return s.habits.find(h=> (h.name||'').trim().toLowerCase() === name.trim().toLowerCase()); }
    function isoDate(d){ const t=new Date(d.getFullYear(), d.getMonth(), d.getDate()); return t.toISOString().slice(0,10); }

    let created = 0, updated = 0;
    plan.forEach(item=>{
      if(!item || !item.name || !item.dates || !item.dates.length) return;
      let h = findByName(item.name);
      if(!h){
        // create habit with empty completion array and targetDays inferred from unique planned dates
        h = { id:'h'+Math.random().toString(36).slice(2,9), name:item.name, color:'#ffd166', start: isoDate(new Date()), days:[], plan:[] };
        s.habits.unshift(h); created++;
      }
      if(!Array.isArray(h.plan)) h.plan = [];
      const set = new Set(h.plan);
      item.dates.forEach(d => set.add(String(d)));
      h.plan = Array.from(set).sort();
      // if user already marked some days as done, keep those intact
      updated++;
    });

    localStorage.setItem(K, JSON.stringify(s));
    // Ask UI to refresh if functions exist
    if(window.renderAll) window.renderAll();
    return created + updated;
  }

  // 5) === Chat hook: route user message -> Mistral -> write calendar ===
  // We wrap existing send() to insert our flow while keeping old behavior for legacy intents.
  const _send = window.send;
  if(_send){
    window.send = async function(){
      const t = (document.getElementById('chat-input')?.value||'').trim();
      if(!t) return _send(); // fall back
      // Push your own message first (like before)
      _send();
      try{
        // 1) Call Mistral with your established prompts
        const raw = await callMistral(t);
        const plan = parseMistralPlan(raw);
        // 2) Write to calendar – NO plan text back to user
        const n = addPlanToCalendar(plan);
        // 3) (Optional) brief confirmation
        if(n>0 && window.pushMsg) window.pushMsg('План добавлен в календарь. Успехов!','bot');
        // Auto switch to habits view so the user sees updated calendars
        if(window.showView) window.showView('view-habit');
      }catch(err){
        console.error(err);
        if(window.pushMsg) window.pushMsg('Не удалось сгенерировать план. Попробуйте ещё раз.','bot');
      }
    };
  }

  // 6) === Optional: mark today as auto-complete if planned and checked ===
  // If a day is planned and user taps the big check, it already toggles `days`.
  // No change needed; the calendar overlay will switch from outline to filled dot.
})();
</script>
